...
def make_cur_feature_vector(x,y,z, Te=1.0):
    ...
    for row in range(x.shape[0]):
        x_r = x.iloc[row,:].values.reshape(1,-1)
        y_r = y.iloc[row,:].values.reshape(1,-1)
        z_r = z.iloc[row,:].values.reshape(1,-1)
        xyz_matrix = np.concatenate((x_r,y_r,z_r), axis=0)
